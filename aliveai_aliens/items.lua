...
minetest.register_tool("aliveai_aliens:ozer_sword", {
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		for i, ob in pairs(minetest.get_objects_inside_radius(d, 4)) do
			if not (ob:get_luaentity() and ob:get_luaentity().type==nil) and aliveai.visiable(pos,ob:getpos()) and not (ob:is_player() and ob:get_player_name()==name) then
...
minetest.register_tool("aliveai_aliens:alien_nrifle", {
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		minetest.add_entity({x=pos.x+(dir.x)*1,y=pos.y+(dir.y)*1,z=pos.z+(dir.z)*1}, "aliveai_aliens:bullet"):setvelocity(d)
...
minetest.register_tool("aliveai_aliens:alien_rifle", {
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		minetest.sound_play("aliveai_aliens_lazer", {pos=pos, gain=1.0, max_hear_distance=10})
		minetest.add_entity({x=pos.x+(dir.x)*3,y=pos.y+(dir.y)*3,z=pos.z+(dir.z)*3}, "aliveai_aliens:bullet"):setvelocity(d)
		minetest.add_entity({x=pos.x+(dir.x)*2,y=pos.y+(dir.y)*2,z=pos.z+(dir.z)*2}, "aliveai_aliens:bullet"):setvelocity(d)
		minetest.add_entity({x=pos.x+(dir.x)*1,y=pos.y+(dir.y)*1,z=pos.z+(dir.z)*1}, "aliveai_aliens:bullet"):setvelocity(d)
		aliveai_aliens.user=nil
		aliveai_aliens.type=nil
		return itemstack
...
minetest.register_tool("aliveai_aliens:alien_homing_rifle", {
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		for i=2,31,1 do
			local pos1={x=pos.x+(dir.x)*i,y=pos.y+(dir.y)*i,z=pos.z+(dir.z)*i}
			...
			for ii, ob in pairs(minetest.get_objects_inside_radius(pos1, 1.5)) do
				if not (ob:get_luaentity() and ob:get_luaentity().type==nil) and aliveai.visiable(pos,ob:getpos()) and not (ob:is_player() and ob:get_player_name()==name) then
					...
					aliveai_aliens.user=user
					minetest.add_entity({x=pos.x+(dir.x)*3,y=pos.y+(dir.y)*3,z=pos.z+(dir.z)*3}, "aliveai_aliens:bullet2")
					return itemstack
				end
			end
		end
...
minetest.register_abm({
	nodenames = {"aliveai_aliens:ufo_spawner","aliveai_aliens:alien_spawner","aliveai_aliens:asteroid","aliveai_aliens:fire"},
	interval = 1.0,
	chance = 1,
	action = function(pos, node, active_object_count, active_object_count_wider)
		local n=minetest.get_node(pos).name

		if n=="aliveai_aliens:fire" then
		...
		if n=="aliveai_aliens:asteroid" and minetest.get_modpath("aliveai_nitroglycerine")~=nil then
			...
			local pos3={}
			local fire={name="aliveai_aliens:fire"}
				...
				elseif not (fn and fn.buildable_to) or aliveai_aliens.atra[id]==1 or i==150 then
					minetest.after(i*0.05, function(pos3,fire)
						...
						aliveai_nitroglycerine.explode(pos3,{
							...
							set="air",
							drops=0,
							place={"aliveai_aliens:fire","air","air","air","air"}
						})
...
minetest.register_node("aliveai_aliens:fire", {
	description = "asteroid fire",
