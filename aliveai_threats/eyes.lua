...
minetest.register_entity("aliveai_threats:eyes",{
	...
	on_activate=function(self, staticdata)
		...
		self.opos=aliveai.roundpos(self.object:getpos())

		minetest.after(0.1, function(self)
			if not self.stat then self.stat=self.object:getpos() end
			local s=3.14
	...
	on_step=function(self, dtime)
		...
		for _, o in ipairs(minetest.get_objects_inside_radius(pos, 10)) do
			...
			local op=o:getpos()
			if not (en and en.aliveai_eyes) and aliveai.visiable(self,o:getpos()) and aliveai.viewfield(self,o) then ob=o break end
		end
